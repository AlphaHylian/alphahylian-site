...
// The head tracks the cursor quickly and directly. Only once the cursor
// asks for more than ~45 degrees of turn does the whole body snap-turn to
// face it; right after that turn completes, body and head both reset back
// to facing straight ahead, then normal head-tracking resumes immediately.
const HEAD_LIMIT_YAW = Math.PI / 4;   // 45 degrees — head's own turning range
const HEAD_LIMIT_PITCH = 0.4;
const HEAD_SPEED = 0.25;              // fast, direct head tracking
const BODY_TURN_SPEED = 0.09;         // visible but quick body turn
const BODY_RESET_SPEED = 0.09;        // snap back to facing forward
const SETTLE_EPSILON = 0.02;

let bodyYaw = 0;
let headYaw = 0;
let headPitch = 0;
let lookMode = 'tracking'; // 'tracking' | 'turning' | 'resetting' | 'dragging'

// If the user grabs the model and drags to manually rotate it (the built-in
// orbit controls), pause cursor-tracking entirely. On release, snap the
// camera back to its default front-on view and reset body/head to neutral,
// then resume normal cursor-tracking.
...
function updateHeadLook(){
  ...
  if(head && wrapper){
    if(lookMode === 'dragging'){
      // Frozen while the user manually rotates the model.
    ...
    // Desired angle (in any direction, left/right/up/down) needed to face the cursor.
    const desiredYaw = Math.max(-1.6, Math.min(1.6, dx)) * 0.95;
    const desiredPitch = Math.max(-1, Math.min(1, dy)) * 0.6;

    if(lookMode === 'tracking'){
      if(Math.abs(desiredYaw) > HEAD_LIMIT_YAW){
        lookMode = 'turning';
      } else {
        const clampedPitch = Math.max(-HEAD_LIMIT_PITCH, Math.min(HEAD_LIMIT_PITCH, desiredPitch));
        headYaw += (desiredYaw - headYaw) * HEAD_SPEED;
        headPitch += (clampedPitch - headPitch) * HEAD_SPEED;
      }
    } else if(lookMode === 'turning'){
      // Head goes neutral while the body turns to face the cursor.
      headYaw += (0 - headYaw) * HEAD_SPEED;
      headPitch += (0 - headPitch) * HEAD_SPEED;
      bodyYaw += (desiredYaw - bodyYaw) * BODY_TURN_SPEED;
      if(Math.abs(desiredYaw - bodyYaw) < SETTLE_EPSILON){
        lookMode = 'resetting';
      }
    } else if(lookMode === 'resetting'){
      // Snap body and head back to facing straight ahead, then resume tracking.
      headYaw += (0 - headYaw) * HEAD_SPEED;
      headPitch += (0 - headPitch) * HEAD_SPEED;
      bodyYaw += (0 - bodyYaw) * BODY_RESET_SPEED;
      if(Math.abs(bodyYaw) < SETTLE_EPSILON && Math.abs(headYaw) < SETTLE_EPSILON){
        bodyYaw = 0;
        headYaw = 0;
        headPitch = 0;
        lookMode = 'tracking';
