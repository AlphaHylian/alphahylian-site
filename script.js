...
// Head tracks the cursor anywhere on the page, smoothly, while the body
// stays free to be drag-rotated via the built-in orbit controls.
function updateHeadLook(){
  ...
  if(head){
    ...
    const dx = Math.max(-1, Math.min(1, (mouse.x - cx) / (rect.width / 2 || 1)));
    const dy = Math.max(-1, Math.min(1, (mouse.y - cy) / (rect.height / 2 || 1)));
    const targetY = dx * 0.5;
    const targetX = dy * 0.35;
